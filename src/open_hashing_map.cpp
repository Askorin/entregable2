...
OpenHashingMap<KeyType, ValueType>::put(KeyType key, ValueType value) {
    size_t idx = this->hash(key);
    for (size_t i = 0; i < table[idx].size(); ++i) {
        if (table[idx][i].key == key) {
...
template <typename KeyType, typename ValueType>
std::vector<std::vector<Entry<KeyType, ValueType>>>
OpenHashingMap<KeyType, ValueType>::getTable() {
    return table;
}
